...
def get_embeddings(texts: list[str]) -> list[list[float]]:
    ...
    if len(texts) > MAX_BATCH_SIZE:
        raise ValueError(f"A maximum of {MAX_BATCH_SIZE} texts can be embedded in one request.")

    cleaned_texts = [prepare_text(text) for text in texts]
    empty_indexes = [index for index, text in enumerate(cleaned_texts) if not text]
    ...
    if any(not embedding for embedding in embeddings):
        raise ValueError("OpenAI returned one or more empty embeddings.")
