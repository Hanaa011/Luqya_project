...
namespace LostFound.Notifications
{
    public class NotificationAppService : ApplicationService, INotificationAppService
    {
        ...
        public async Task<PagedResultDto<NotificationDto>> GetMyListAsync(PagedAndSortedResultRequestDto input)
        {
            if (!CurrentUser.IsAuthenticated || CurrentUser.Id == null)
            {
                return new PagedResultDto<NotificationDto>(0, new System.Collections.Generic.List<NotificationDto>());
            }

            var queryable = (await _notificationRepository.GetQueryableAsync())
                .Where(n => n.IdentityUserId == CurrentUser.Id);
